library(readxl)
library(class)
library(gmodels)
library(caret)
library(ggplot2)
library(reshape2)
library(DataExplorer)

df <- read_excel("Downloads/Data for publication_072820.xlsx")
View(df)
head(df)

#Clean Data
sum(is.na(df))
colSums(is.na(df))
df1 <- na.omit(df)
sum(is.na(df1))

#Exploratory Data Analysis 
View(df1)
summary(df1)
str(df1)

hist(df1$age, main = 'Age of Covid-19 Patients', xlab = 'Age', col = 'blue')
hist(df1$race, main = 'Race of Covid-19 Patients', xlab = 'Race', col = 'blue')
hist(df1$sex, main = 'Sex of Covid-19 Patients', xlab = 'Sex', col = 'blue')

#Normalize Data
df2 <- df1
normalize <- function(x) {
  return ((x - min(x)) / (max(x) - min(x)))
}
df2[1:19] <- as.data.frame(lapply(df2[1:19], normalize))

#KNN
View(df2)
df2$Status_comp = NULL
df2$Seizure_comp <- as.factor(df2$Seizure_comp)
summary(df2)
str(df2)

set.seed(789)
ind <- sample(2, nrow(df2), replace=TRUE, prob=c(0.7, 0.3))
train <- df2[ind==1,]
test <- df2[ind==2,]

k1 <- knn(train = train[1:18], test = test[1:18], cl = train$Seizure_comp, k = 1)
CrossTable(x = test$Seizure_comp, y = k1, prop.chisq = FALSE)
table(test$Seizure_comp, k1)
confusionMatrix(table(test$Seizure_comp, k1))

k2 <- knn(train = train[1:18], test = test[1:18], cl = train$Seizure_comp, k = 2)
CrossTable(x = test$Seizure_comp, y = k2, prop.chisq = FALSE)
table(test$Seizure_comp, k2)
confusionMatrix(table(test$Seizure_comp, k2))

k3 <- knn(train = train[1:18], test = test[1:18], cl = train$Seizure_comp, k = 3)
CrossTable(x = test$Seizure_comp, y = k3, prop.chisq = FALSE)
table(test$Seizure_comp, k3)
confusionMatrix(table(test$Seizure_comp, k3))

#Logistic Regression 
table(df2$Seizure_comp)

log <- glm(Seizure_comp ~., data = df2, family = binomial())
summary(log)

confint(log)
coef(log)
exp(coef(log))
exp(confint(log))

fit_model <- train(Seizure_comp ~., data = train, method = 'glm', family = binomial())
pred <- predict(fit_model, test)
head(pred)
table(pred, test$Seizure_comp)
confusionMatrix(table(pred, test$Seizure_comp))

  
  
  
  
  
  

